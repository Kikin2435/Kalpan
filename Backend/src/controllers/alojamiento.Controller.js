...
export const crearAlojamiento = async (req, res) => {
  try {
    const { titulo_anuncio, ubicacion, precio, tipo, no_habitacion, no_banios, superficie, descripcion, amenidades, servicios, estacionamiento, reglas, status } = req.body;

    const newAlojamiento = await Alojamiento.create({
      titulo_anuncio, ubicacion, precio, tipo, no_habitacion, no_banios, superficie, descripcion, amenidades, servicios, estacionamiento, reglas, status
    });
